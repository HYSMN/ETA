...
def train_model(network_data, feature_dic):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_nodes):
            ...
            if epoch == 3: 
                with io.open('3rest0.txt', mode="a", encoding='utf-8') as data0:  
                    data0.write(unicode(i))
                    data0.write(' ')
                    data0.write(unicode(index2word[i]))
                    data0.write(unicode(node_emb[0].cpu().detach().numpy()))
                    data0.write('\n')
                    data0.close()
                with io.open('3rest1.txt', mode="a", encoding='utf-8') as data1:  
                    data1.write(unicode(i))
                    data1.write(' ')
                    data1.write(unicode(index2word[i]))
                    data1.write(unicode(node_emb[1].cpu().detach().numpy()))
                    data1.write('\n')
                    data1.close()
                with io.open('3rest2.txt', mode="a", encoding='utf-8') as data2:  
                    data2.write(unicode(i))
                    data2.write(' ')
                    data2.write(unicode(index2word[i]))
                    data2.write(unicode(node_emb[2].cpu().detach().numpy()))
                    data2.write('\n')
                    data2.close()
                with io.open('3rest3.txt', mode="a", encoding='utf-8') as data3:  
                    data3.write(unicode(i))
                    data3.write(' ')
                    data3.write(unicode(index2word[i]))
                    data3.write(unicode(node_emb[3].cpu().detach().numpy()))
                    data3.write('\n')
                    data3.close()
                with io.open('3rest4.txt', mode="a", encoding='utf-8') as data4:  
                    data4.write(unicode(i))
                    data4.write(' ')
                    data4.write(unicode(index2word[i]))
                    data4.write(unicode(node_emb[4].cpu().detach().numpy()))
                    data4.write('\n')
                    data4.close()
                with io.open('3rest5.txt', mode="a", encoding='utf-8') as data5:  
                    data5.write(unicode(i))
                    data5.write(' ')
                    data5.write(unicode(index2word[i]))
                    data5.write(unicode(node_emb[5].cpu().detach().numpy()))
                    data5.write('\n')
                    data5.close()
                with io.open('3rest6.txt', mode="a", encoding='utf-8') as data6:
                    data6.write(unicode(i))
                    data6.write(' ')
                    data6.write(unicode(index2word[i]))
                    data6.write(unicode(node_emb[6].cpu().detach().numpy()))
                    data6.write('\n')
                    data6.close()
...
if __name__ == "__main__":
    ...
    training_data_by_type = load_training_data(file_name + "/train3.txt")
    ...
    #     file_name + "/test.txt"
    # )
    train_model(training_data_by_type, feature_dic)
